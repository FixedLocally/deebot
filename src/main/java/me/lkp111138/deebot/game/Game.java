...
public class Game {
    ...
    public Game(Message msg, int chips, int wait, GroupInfo groupInfo) throws ConcurrentGameException {
        ...
        if (maint_mode) {
            ...
            this.execute(new SendMessage(gid, String.format(getTranslation("GAME_START_ANNOUNCEMENT"), msg.from().id(), msg.from().firstName())).parseMode(ParseMode.HTML), new EmptyCallback<>());
            return;
        }
